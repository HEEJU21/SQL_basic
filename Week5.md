...
~~~
3번. login_date는 보통 DATE 타입이고 '2021'은 문자열(YYYY) 형식.

DB에 따라 '2021'을 자동 변환하지 못할 수 있고, 설령 가능하더라도 '2021-01-01'과 같은 날짜로 변환되지 않아 원하는 결과(2021년 전체)가 아님.

4번. 날짜 범위를 지정하는 표준 SQL 구문으로 완벽함.

'2021-12-31' 포함 여부도 BETWEEN은 양쪽 경계를 포함하므로 문제 없음.
~~~
